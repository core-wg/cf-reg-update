...
Furthermore, this document reserves Content-Format identifiers 64998 and 64999 for use in documentation.
...
This document updates {{-coap}} by modifying the registration procedures for the "CoAP Content-Formats" registry to mitigate the risk of unintentional or malicious errors.
...
It also removes a note that was added to the registry as a temporary patch ({{temp-note-removal}}), adds a new note concerning temporary registrations ({{new-note-add}}) and reserves Content-Format IDs 64998 and 64999 for documentation ({{reserve-64999}}).
...
For example, a Smart Energy application payload carried as Concise Binary Object Representation (CBOR) might request a more specific type like application/se+cbor.
...
Content-Format ID 64998 where the "Content Coding" field is left empty.
...
## Reserving Content-Format Identifiers 64998 and 64999 for Documentation {#reserve-64999}

IANA is instructed to reserve Content-Format identifiers 64998 and 64999 for use in documentation.
